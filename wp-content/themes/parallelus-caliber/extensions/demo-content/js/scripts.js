...
jQuery(window).load(function() {
	...
	function ajaxImportThemeContent() {
		jQuery.ajax({
			type: 'POST',
			url: ajaxurl,
			data: {
			...
			error: function(XMLHttpRequest, textStatus, errorThrown){
				...
				if (typeof XMLHttpRequest.status !== 'undefined' && XMLHttpRequest.status == '504') {
					// Gateway Timeout
					// try once again
					console.log('504. One more attempt');
					ajaxImportThemeContent();
				} else {
					import_content_running = false;
					jQuery('.starter-kits .button').removeAttr('disabled'); // enable the buttons again.
					response = jQuery('#DemoImportNotice').children('.response-2').text();
					jQuery('#DemoImportNotice')
						.addClass('error')
						.removeClass('updated active')
						.children('p').css('display','none').end()
						.children('.response-2')
						.text(response.replace("{folder}",alias))
						.fadeIn('slow').end();
				}
