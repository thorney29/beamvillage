...
class Demo_Content_Admin_Object extends Runway_Admin_Object {
			...
			if ( file_exists( $starterkit_dir ) ) {
				...
				if (isset($files) && !empty($files)) {
					...
					foreach ( $files as $key => $file ) {
						...
						$value = json_decode( $wp_filesystem->get_contents( $file ), true ); // FFS
						// $value = json_decode( call_user_func( 'file' .'_get_'. 'contents',  $file ), true ); // FFS
					...
					if ( (isset($frontPage) && $frontPage) || (isset($postsPage) && $postsPage) ) {
						...
						if (isset($postsPage) && $postsPage) {
							$postsPageID = (!is_numeric($postsPage)) ? $this->get_ID_by_slug($postsPage) : $postsPage;
						}
	...
	public function importThemeContent() {
		...
		if (isset($files) && !empty($files)) {
			...
			// fix curl options
			add_action( 'requests-curl.before_send', array( $this, 'fix_curl_options' ) );
	...
	// redefine Referer (for correct image redirects)
	public function fix_curl_options( $handle ) {
		if ( is_array( $handle ) ) {
			$handle = $handle[0];
		}
		curl_setopt( $handle, CURLOPT_REFERER, get_site_url() );
	}
	...
	public function displayStarterKit( $name = '', $alias = '', $menu = '', $image = '' ) {
		...
		'<div class="starterkit" tabindex="0">'.
			...
			'<div class="starterkit-actions">'.
				'<a class="button apply-kit" data-alias="'.$alias.'" data-menu="'.$menu.'" href="'. wp_nonce_url( $this->self_url('default', array('action'=>'apply-kit', 'alias'=>$alias)), 'starter_kit') .'">'. __('Apply', 'runway') .'</a>'.
				' &nbsp; '.
				'<a class="button button-primary apply-kit-with-content" data-alias="'.$alias.'" data-menu="'.$menu.'" href="'. wp_nonce_url( $this->self_url('default', array('action'=>'apply-kit-with-content', 'alias'=>$alias)), 'starter_kit') .'">'. __('Apply with Demo Content', 'runway') .'</a>'.
			'</div>'.
		'</div>';
	}
