		...
		switch ($action) {
			...
			case 'delete-backup':{
				...
				if (isset($alias) && !empty($alias)) {
					...
					// success or error
					if ($confrim) {
						$message = __('The backup has been deleted.', 'runway');
						$message_class = 'updated';
					} else {
						__('The backup could not be deleted.', 'runway');
						$message_class = 'error';
					}
				}
			} break;
			...
			case 'restore-backup':{
				...
				if (isset($alias) && !empty($alias)) {
					...
					// success or error
					if ($confrim) {
						$message = __('The backup has been restored.', 'runway');
						$message_class = 'updated';
					} else {
						__('The backup could not be restored.', 'runway');
						$message_class = 'error';
					}				}
			} break;
