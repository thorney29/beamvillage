...
if (!count($history) || $action == 'rebuild') {
	...
	if ($alone_package_download_url && $child_package_download_url && $most_recent)
		echo '<div id="message" class="updated below-h2"><p>'. __('New download package created', 'runway').'.</p></div>';
} else {
...
?>
<h3><?php _e('Most Recent', 'runway') ?></h3>

<table class="widefat">
	<thead>
		<tr>
			<th><?php _e('Date', 'runway') ?></th>
			<th><?php _e('Standalone Theme', 'runway') ?></th>
			<th><?php _e('Child Theme', 'runway') ?></th>
			<th><?php _e('Tags', 'runway') ?></th>
		</tr>
	</thead>
	<tbody>
		<tr>
...
if ( $current_package['a_hash'] ) {
	?>
				...
				<p><a href="<?php echo esc_url( home_url() . '/wp-content/themes/' . $_REQUEST['name'] . '/download/' . $current_package['a_file'] ); ?>" class="button-primary"><?php _e('Download', 'runway') ?></a></p>
				<p class="checksum"><?php echo __('Checksum', 'runway') .": <span class='code'>". $current_package['a_hash']; ?></span></p>
				<?php
			}
else {
...
if ( $current_package['c_hash'] ) { ?>
				<p><a href="<?php echo esc_url( home_url() . '/wp-content/themes/' . $_REQUEST['name'] . '/download/' . $current_package['c_file'] ); ?>" class="button-primary"><?php _e('Download', 'runway') ?></a></p>
...
<div class="tags-dialog">
	<fieldset>
	    <label for="tags-edit"><?php echo __('Add tags to this package? (optional)','runway'); ?></label><br/>
	...
	<br>
	<button id="tags-save" class="button accept-changes button-primary"><?php _e( 'Update', 'runway' ); ?></button>
</div>
	...
	<table class="widefat">
		<thead>
			<tr>
				...
				<th><?php _e('Child Theme', 'runway') ?></th>
				<th><?php _e('Tags', 'runway') ?></th>
				<th><?php _e('Edit', 'runway') ?></th>
				<th><?php _e('Delete', 'runway') ?></th>
			</tr>
		</thead>
		<tbody><?php
		...
		if ( !$history ) { ?>
				<tr>
					<th colspan="3">
						...
						<div style="text-align: center;"><?php _e('Have no packages to download', 'runway'); ?></div>
			...
			foreach ( $history as $package ) {
				...
				<?php
					...
					<td>
						<p><a href="<?php echo esc_url( $developer_tools->self_url('edit-tags-package').'&name='.$_REQUEST['name'].'&package='.$package['exp'] ); ?>" class="link-tags-edit"><?php _e('Edit', 'runway'); ?></a></p>
					</td>
					<td>
						<!--.'&name=liftoff&action=delete-package&package='.$package['exp']-->
						<p><a href="<?php echo esc_url( $developer_tools->self_url('confirm-del-package').'&name='.$_REQUEST['name'].'&package='.$package['exp'] ); ?>" ><?php _e('Delete', 'runway'); ?></a></p>
					</td>
				</tr>
			<?php }
		}?>
