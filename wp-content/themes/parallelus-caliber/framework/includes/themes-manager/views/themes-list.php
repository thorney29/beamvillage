...
$popup_message = '<h2>'. __( 'Activate new theme?', 'runway' ) .'</h2>';
...
if ( isset( $_GET['activate-default'] ) && $_GET['activate-default'] == 'activate' && ($httpReferer == $themesActivate || strstr($httpReferer, 'action=upload-theme') ) ) {
	...
	$popup_message .= '<h2>'. __( 'Welcome to Runway!', 'runway' ). '</h2><p>'. __( 'We recommend using Runway with an active child theme. Would you like to automatically activate the default child theme?', 'runway' ) .'</p>';
	...
	if(is_multisite() && is_admin() && isset($options)){
		...
		<div class="enable-theme-popup themeActionsPupup" style="display:none;">
			...
			<?php echo __('Enable default theme for a network', 'runway'); ?>?

			<p class="bottom-panel">
				<a href="<?php echo esc_url($ms_enable_theme_link); ?>" class="enable-theme button"><?php _e( 'Enable', 'runway' ); ?></a>
				&nbsp;<a href="javascript: void(0);" class="button"><?php _e( 'Cancel', 'runway' ); ?></a>
			</p>
...
<div class="duplicate-theme-popup themeActionsPupup">
	<p><?php _e( 'Enter a new folder name:', 'runway' ); ?></p>
	<p>
	...
	<p>
		<a href="<?php echo admin_url('admin.php?page=themes&navigation=duplicate-theme&_wpnonce='.wp_create_nonce('duplicate-theme').'&name='); ?>" class="submit-theme-new-folder button-primary" title="<?php _e( 'Duplicate', 'runway' ); ?>"><?php _e( 'Duplicate', 'runway' ); ?></a> <a href="#" class="button cancel-duplicate" title="<?php _e( 'Cancel', 'runway' ); ?>"><?php _e( 'Cancel', 'runway' ); ?></a>
	</p>

</div>

<div class="loader themeActionsPupup" style="display: none;">
	<p><?php _e( 'Loading...', 'runway' ); ?></p>
	<img src="<?php
...
<div class="wrap">
	...
	<?php
		...
		wp_localize_script( 'themes-manager-themes', '_wpThemeSettings', array(
			...
			'settings' => array(
				...
				'confirmDelete' => __( "Are you sure you want to delete this theme?\n\nClick 'Cancel' to go back, 'OK' to confirm the delete.", 'runway' ),
				'adminUrl'      => parse_url( admin_url(), PHP_URL_PATH ),
			),
			'l10n' => array(
				'addNew' => __( 'Add New Theme', 'runway' ),
				'search'  => __( 'Search Installed Themes', 'runway' ),
				'searchPlaceholder' => __( 'Search installed themes...', 'runway'  ), // placeholder (no ellipsis)
			),
		) );
		...
		<div class="themes">
			...
			<div class="theme add-new-theme">
				<a href="<?php echo admin_url('admin.php?page=themes&navigation=new-theme'); ?>">
					...
					<h3 class="theme-name"><?php echo __('Create New Theme', 'runway'); ?></h3>
				</a>
			</div>
		</div>
		<br class="clear">
	...
	<h3 class="adminTitle"><?php _e('Other Runway Themes', 'runway'); ?> &nbsp; <a class="add-new-h2" href="<?php echo admin_url('admin.php?page=directory'); ?>"><?php _e('More Themes', 'runway'); ?></a></h3>
	<?php
		...
		<div class="themes">
			...
			<div class="theme add-new-theme runway-find-more-themes">
				<a href="<?php echo esc_attr(admin_url('admin.php?page=directory&addons=themes')); ?>">
					<div class="theme-screenshot"><span></span></div>
					<h3 class="theme-name"><?php echo __('Find More Themes', 'runway'); ?></h3>
				</a>
			</div>
		</div>
		<br class="clear">
	...
	<script id="tmpl-theme-modal" type="text/x-jquery-tmpl">
		...
		<div class="theme-wrap">
			<div class="theme-header">
				<button class="left dashicons dashicons-no"><span class="screen-reader-text"><?php _e( 'Show previous theme', 'runway' ); ?></span></button>
				<button class="right dashicons dashicons-no"><span class="screen-reader-text"><?php _e( 'Show next theme', 'runway' ); ?></span></button>
				<button class="close dashicons dashicons-no"><span class="screen-reader-text"><?php _e( 'Close overlay', 'runway' ); ?></span></button>
			</div>
			<div class="theme-about">
				...
				<div class="theme-info">
					{{if active == true }}
						<span class="current-label"><?php _e( 'Current Theme', 'runway' ); ?></span>
					{{/if}}
					<h3 class="theme-name">${name}<span class="theme-version"><?php printf( __( 'Version: %s', 'runway' ), '${version}' ); ?></span></h3>
					<h4 class="theme-author"><?php printf( __( 'By %s', 'runway' ), '{{html authorAndUri}}' ); ?></h4>

					{{if hasUpdate == true}}
					<div class="theme-update-message">
						<h4 class="theme-update"><?php _e( 'Update Available', 'runway' ); ?></h4>
						${update}
					</div>
					{{/if}}
					...
					{{if parent != null && parent != false}}
						<p class="parent-theme"><?php printf( __( 'This is a child theme of %s.', 'runway' ), '<strong>{{html parent}}</strong>' ); ?></p>
					{{/if}}

					{{if tags != null && tags != false}}
						<p class="theme-tags"><span><?php _e( 'Tags:', 'runway' ); ?></span> ${tags}</p>
					{{/if}}
				</div>
			</div>
			...
			<div class="theme-actions">
				<div class="active-theme">
					<a href="${actions.customize}" class="button button-primary customize load-customize hide-if-no-customize"><?php _e( 'Customize', 'runway' ); ?></a>
				</div>
				<div class="inactive-theme">
					{{if actions.activate != false && actions != ""}}
						<a href="${actions.activate}" class="button button-primary activate"><?php _e( 'Activate', 'runway' ); ?></a>
					{{/if}}
					<a href="${actions.customize}" class="button button-secondary load-customize hide-if-no-customize"><?php _e( 'Live Preview', 'runway' ); ?></a>
					<a href="${actions.preview}" class="button button-secondary hide-if-customize"><?php _e( 'Preview', 'runway' ); ?></a>
				</div>

				{{if active == false && actions.delete != false && actions.delete != "" }}
					<a href="${actions.delete}" class="button button-secondary delete-theme"><?php _e( 'Delete', 'runway' ); ?></a>
				{{/if}}
			</div>
		</div>
		</div>
